...
new URLParser("http://localhost:9000/blah/bleh?foo=bar&fox=baz%20bee#frag").URL.run().recover  {
  case e: ParseError => {
    s"""${e.formatExpectedAsString} expected at column ${e.position.column}
       |${e.formatTraces}""".stripMargin
  }
}.get
